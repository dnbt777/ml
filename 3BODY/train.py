...
def get_loss(new_policy_model_params, old_policy_model_params, key,
  ...
  old_policy, new_policy, outcome_rewards = jax.vmap(jax.vmap(run_single_trajectory_g))(random_keys)
  ...
  advantages = (outcome_rewards - jnp.mean(outcome_rewards, axis=-1, keepdims=True)) / jnp.std(outcome_rewards, axis=-1, keepdims=True) # (batch, g)
...
print(end - jit_end)
print(f"steps/sec:{train_iters*trajectory_steps*G / (end - jit_end)}")
