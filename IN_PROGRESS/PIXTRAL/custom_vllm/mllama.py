# mllama, modified to print outputs
print("INJECTED CUSTOM VLLM")
...
print("Determinism enforced:      ", torch.are_deterministic_algorithms_enabled())

# Optional: for PyTorch 2.3+
if hasattr(torch.backends, "mha"):
    print("MHA backend module available.")
"""
...
class MllamaMultiModalProcessor(EncDecMultiModalProcessor[MllamaProcessingInfo]
    ...
    def apply(
        ...
        if mm_data:
            ...
            print("MultiModalProcessor | self.info", self.info)
            print("MultiModalProcessor | hf_processor", hf_processor)
    ...
    def _call_hf_processor(
        ...
        if mm_data:
            # print("MMA DATA", mm_data)
            num_tiles = [
                self.info.get_num_tiles_per_image(img.height, img.width)
                for img in mm_data["images"]
            ]
            #tdiff.capture(mm_data["images"][0], name="pre_preprocessing.img", project="vllm")
...
class ColumnParallelConv2dPatch(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self._unfold(x)
        x = x.permute(0, 2, 1)
        tdiff.capture(x, name="pre_convolve.prematmul_tiles", project="vllm")
        tdiff.capture(self._linear.weight, name="pre_convolve.patch_embedding_weight", project="vllm")
        print("LINEAR QUANT CONFIG", self._linear.quant_method)
        x, _ = self._linear(x)
        tdiff.capture(x, name="post_convolve.hidden_state", project="vllm")
        return x
...
class MllamaVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        #print("MllamaVisionSdpaAttention | Q first head | ", q.shape, q.dtype, q[0][0][0]) 
        k = k.view(k.shape[0], k.shape[1], self.num_local_heads,
                   self.head_dim).transpose(1, 2)
        #print("MllamaVisionSdpaAttention | K first head | ", k.shape, k.dtype, k[0][0][0]) 
        v = v.view(v.shape[0], v.shape[1], self.num_local_heads,
                   self.head_dim).transpose(1, 2)
        
        #print("MllamaVisionSdpaAttention | V first head | ", v.shape, v.dtype, v[0][0][0]) 
        #print("MllamaVisionSdpaAttention | K | ", k.shape) 
        #print("MllamaVisionSdpaAttention | V | ", v.shape) 

        # TODO: remove padding in image encoder
        #write_array_to_txt(q, "outputs/vllm_q.txt")
        #write_array_to_txt(k, "outputs/vllm_k.txt")
        #write_array_to_txt(v, "outputs/vllm_v.txt")
        # print("QKV", torch.sum(q), torch.sum(k), torch.sum(v))
        attn_output = F.scaled_dot_product_attention(q,
                                                     k,
                                                     v,
                                                     attn_mask=attention_mask,
                                                     dropout_p=0.0)
        #print("attn output first row", attn_output.shape, attn_output[0][0][0])

        attn_output = attn_output.transpose(1, 2).contiguous()
        attn_output = attn_output.reshape(attn_output.shape[0],
                                          attn_output.shape[1], -1)
        # print("pre-o", attn_output.shape, attn_output.dtype, attn_output[0][0])
        output, _ = self.o_proj(attn_output)
        # print("MllamaVisionSdpaAttention | attn_output | ", output.shape, output[0][0]) 
        return output
...
class MllamaVisionEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # print("MllamaVisionEncoderLayer | start state ", hidden_state.shape, hidden_state)
        hidden_state = self.input_layernorm(hidden_state)
        #print("MllamaVisionEncoderLayer | post input layernorm state ", hidden_state.shape, hidden_state)
        #print("MllamaVisionEncoderLayer | post input layernorm state ", hidden_state.shape, hidden_state.dtype, torch.sum(hidden_state, axis=-1), torch.sum(hidden_state, axis=-1).shape)
        print(torch.sum(hidden_state, axis=-2))
        hidden_state = self.self_attn(hidden_state,
                                      attention_mask=attention_mask)
        #print("MllamaVisionEncoderLayer | forward | post attn layer:", hidden_state.shape, hidden_state)
        ...
        #print("MllamaVisionEncoderLayer | postattn residual 1 ", hidden_state.dtype, hidden_state.shape, hidden_state[0])
        hidden_state = self.mlp(hidden_state)
        #print("MllamaVisionEncoderLayer | postattn residual postffw ", hidden_state.dtype, hidden_state.shape, hidden_state[0])
...
class MllamaVisionModel(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor,
        ...
        pixel_values = pixel_values.reshape(
            batch_size * num_concurrent_media * num_tiles, num_channels,
            height, width)
        tdiff.capture(pixel_values, name="pre_convolve.pixel_values", project="vllm")
        aspect_ratio_ids = aspect_ratio_ids.reshape(
            batch_size * num_concurrent_media, -1)
        # print("MllamaVisionModel | forward | pixel values:", pixel_values.shape, aspect_ratio_ids, pixel_values) # works
        # print("MllamaVisionModel | forward | pixel values:", pixel_values.shape, aspect_ratio_ids)
        ...
        tdiff.capture(hidden_state, name="post_tile_pos_embed.hidden_state", project="vllm")
        ...
        num_patches += 1
        tdiff.capture(hidden_state, name="post_cls_token.hidden_state", project="vllm")
        ...
        hidden_state = self.gated_positional_embedding(hidden_state,
                                                       aspect_ratio_ids)
        tdiff.capture(hidden_state, name="post_gated_pos_embed.hidden_state", project="vllm")

        # apply encoder
        hidden_state = self.layernorm_pre(hidden_state)
        tdiff.capture(hidden_state, name="post_layernorm.hidden_state", project="vllm")
        ...
        hidden_state = hidden_state.view(batch_size * num_concurrent_media, -1,
                                         dim)
        tdiff.capture(attention_mask[0][0], name="pre_local_layers.attention_mask", project="vllm")
        tdiff.capture(hidden_state[0], name="pre_local_encoder.hidden_state", project="vllm")
        output = self.transformer(
            hidden_state,
            attention_mask=attention_mask,
        )
        hidden_state, intermediate_hidden_states = output[0], output[1]
        tdiff.capture(hidden_state, name="post_local_encoder.hidden_state", project="vllm")
        intermediate_hidden_states = torch.stack(intermediate_hidden_states,
                                                 dim=-1)

        # apply global encoder
        hidden_state = self.layernorm_post(hidden_state)
        tdiff.capture(hidden_state, name="post_after_local_layernorm.hidden_state", project="vllm")
        hidden_state = hidden_state.reshape(batch_size * num_concurrent_media,
                                            num_tiles,
        ...
        hidden_state = self.post_tile_positional_embedding(
            hidden_state, aspect_ratio_ids)
        tdiff.capture(hidden_state, name="post_tile_embed.hidden_state", project="vllm")
        
        hidden_state = hidden_state.reshape(
            batch_size * num_concurrent_media,
            num_tiles * (num_patches + num_padding_patches), dim)
        tdiff.capture(hidden_state, name="pre_global_encoder.hidden_state", project="vllm")
        hidden_state = self.global_transformer(
            hidden_state, attention_mask=attention_mask)[0]
        tdiff.capture(hidden_state, name="post_global_encoder.hidden_state", project="vllm")
        hidden_state = hidden_state.reshape(batch_size * num_concurrent_media,
                                            num_tiles,
                                            num_patches + num_padding_patches,
                                            dim)
        ...
        tdiff.capture(hidden_state, name="vision_processing_output.hidden_state", project="vllm")
        return hidden_state
