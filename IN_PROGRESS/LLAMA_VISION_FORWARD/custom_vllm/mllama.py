...
class MllamaMultiModalProcessor(EncDecMultiModalProcessor[MllamaProcessingInfo]
    ...
    def _call_hf_processor(
        ...
        if mm_data:
            ...
            print("PIXEL VAL SHAPE", processed_outputs["pixel_values"].shape)
            print("FIRST TILE", processed_outputs["pixel_values"].shape, processed_outputs["pixel_values"][0][0][0])
            print("SECOND TILE", processed_outputs["pixel_values"].shape, processed_outputs["pixel_values"][0][0][1])
            print("THIRD TILE", processed_outputs["pixel_values"].shape, processed_outputs["pixel_values"][0][0][2])
            print("FOURTH TILE", processed_outputs["pixel_values"].shape, processed_outputs["pixel_values"][0][0][3])
            print("ASPECT RATIO ID", processed_outputs["aspect_ratio_ids"])
...
class MllamaVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        #print("MllamaVisionSdpaAttention | Q | ", q.shape, q) 
        ...
        attn_output = F.scaled_dot_product_attention(q,
                                                     ...
                                                     dropout_p=0.0)
        #print("MllamaVisionSdpaAttention | attn_output | ", attn_output.shape, attn_output) 
...
class MllamaVisionModel(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor,
        ...
        # patch embedding
        print("PATCH EMBED SHAPE PRE CONV", pixel_values.shape)
        ...
        print("MllamaVisionModel | forward | post conv:", hidden_state.shape) # DONE
        ...
        print("MllamaVisionModel | forward | post pretile embed:", hidden_state.shape, hidden_state.dtype, hidden_state)
        ...
        num_patches += 1
        print("MllamaVisionModel | forward | post cls token:", hidden_state.shape, hidden_state)
        ...
        hidden_state = self.gated_positional_embedding(hidden_state,
                                                       aspect_ratio_ids)
        print("MllamaVisionModel | forward | post pos embed:", hidden_state.shape, hidden_state)
        ...
        hidden_state = hidden_state.view(batch_size * num_concurrent_media, -1,
                                         dim)
        print("MllamaVisionModel | forward | pre local transformer:", hidden_state.shape, hidden_state)
        print("ATTENTION MASK: Masked:", torch.count_nonzero(attention_mask), "/", torch.numel(attention_mask), attention_mask.shape, torch.min(attention_mask), torch.max(attention_mask))
        stride = 100
        
        torch.set_printoptions(profile="full")
        print("ATTENTION MASK:", attention_mask[0][0][::stride, ::stride])
        #print("ATTENTION_MASK FIRST ROW", attention_mask[0][0][0])
        #print("ATTENTION_MASK LAST ROW", attention_mask[0][0][-1])
        #print("ATTENTION_MASK ROW SUM", torch.sum(attention_mask[0][0], axis=0), torch.sum(attention_mask[0][0], axis=0).size())
        #print("ATTENTION_MASK FIRST COL", attention_mask[0][0].T[0])
        #print("ATTENTION_MASK LAST COL", attention_mask[0][0].T[-1])
        #print("ATTENTION_MASK COL SUM", torch.sum(attention_mask[0][0], axis=1), torch.sum(attention_mask[0][0], axis=1).size())
        ...
        print("MllamaVisionModel | forward | post local transformer:", hidden_state.shape, hidden_state)
        intermediate_hidden_states = torch.stack(intermediate_hidden_states,
                                                 dim=-1)

        # apply global encoder
        hidden_state = self.layernorm_post(hidden_state)
        print("MllamaVisionModel | forward | post layernorm:", hidden_state.shape, hidden_state)
        hidden_state = hidden_state.reshape(batch_size * num_concurrent_media,
                                            num_tiles,
        ...
        hidden_state = self.post_tile_positional_embedding(
            hidden_state, aspect_ratio_ids)
        print("MllamaVisionModel | forward | post posttile embed:", hidden_state.shape, hidden_state)
        hidden_state = hidden_state.reshape(
        ...
        print("MllamaVisionModel | forward | post global transformer:", hidden_state.shape, hidden_state)
        hidden_state = hidden_state.reshape(batch_size * num_concurrent_media,
                                            num_tiles,
                                            num_patches + num_padding_patches,
                                            dim)
